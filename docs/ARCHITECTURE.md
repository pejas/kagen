...
5. **Single Responsibility Orchestration**: Host-side orchestration is split into focused coordinators (runtime bootstrap, devfile validation, forgejo sync, agent launch) to reduce coupling inside `internal/cmd`.
...
- **Cluster Coordinator (`internal/cluster`)**: Uses client-go plus shared adapters to reconcile Namespaces, PVCs, and Pods from `devfile.yaml`; performs workspace bootstrap init injection and agent runtime env injection.
...
- **Agent Pod**: Generated from `devfile.yaml`, mounts workspace volume at `/projects/workspace`, and injects agent-specific env/config; bootstrap init container clones Forgejo repo.
...
## Data Flow
1. **Init**: `kagen init` bootstraps `devfile.yaml` and `.kagen.yaml` using the selected agent template.
2. **Start**: `kagen` entrypoint runs coordinators sequentially:
   - Runtime coordinator: dependency check + ensure Colima profile running.
   - Config coordinator: load and validate config, including proxy allowlist.
   ...
   - Cluster coordinator: generate Pod from devfile, inject workspace sync init container, ensure PVCs, create Pod.
...
3. **Work**: Agent TUI operates inside the Pod.
4. **Checkpoint**: Agent-side pushes to Forgejo; host-side provenance recorded.
5. **Review**: `kagen open` uses Forgejo coordinator + port-forward to present review URL.
6. **Pull**: `kagen pull` uses Git engine plus port-forward to merge `kagen/<branch>` back into host branch, safeguarding with optional WIP commit.
...
1. **Orchestration Decomposition**
   - Extract runtime, devfile validation, forgejo sync, and agent launch into discrete coordinator structs invoked from `internal/cmd/root.go`.
